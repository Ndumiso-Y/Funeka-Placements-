        ...
        <Container className="relative z-10 py-20">
          <div className="max-w-4xl">
            <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.25em] mb-10">
              ...
              <span className="text-funeka-pop">/</span>
              <span className="text-white/60">{job.title}</span>
            </div>
            
            <div className="flex flex-wrap items-center gap-4 mb-10">
              <span className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border-2 shadow-2xl ${
                isClosed ? "bg-white/5 text-white/30 border-white/10" : "bg-funeka-pop/20 text-funeka-pop border-funeka-pop/30 backdrop-blur-md"
              }`}>
            ...
            {!isClosed && (
              <div className="mt-12 flex flex-wrap gap-5">
                <Button as={NavLink} to="/apply" variant="pop" className="px-12 py-5 text-lg shadow-funeka-pop/40">
            ...
            {isClosed && (
              <div className="mt-12">
                <p className="text-white/60 font-medium italic border-l-4 border-funeka-pop/50 pl-6 py-3 bg-white/5 rounded-r-2xl">
                ...
                <ul className="grid gap-5">
                  {job.responsibilities.map((r, i) => (
                    <li key={i} className="flex gap-6 p-6 rounded-[1.5rem] border-2 border-funeka-divider bg-funeka-bg/50 hover:bg-white hover:border-funeka-brand/30 transition-all group">
                      <span className="flex-shrink-0 w-10 h-10 rounded-xl bg-funeka-anchor text-funeka-pop flex items-center justify-center text-sm font-black group-hover:scale-110 transition-transform">{i+1}</span>
                ...
                {!isClosed ? (
                  <div className="mt-12 space-y-6">
                    ...
                    <div className="p-4 rounded-2xl bg-funeka-bg text-center">
                      ...
                      <a href={`mailto:${job.applyEmail}`} className="text-xs font-black text-funeka-anchor hover:text-funeka-pop transition-colors">{job.applyEmail}</a>
                    </div>
                  </div>
                ) : (
